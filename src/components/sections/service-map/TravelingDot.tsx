...
export default function TravelingDot({ from, to, isInView, delay }: Props) {
  ...
  return (
    <motion.span
      ...
      animate={{ left: xs, top: ys, opacity: [0, 1, 1, 0] }}
      transition={{
        left: { duration: 3.2, delay: delay + 1.3, repeat: Infinity, ease: "linear" },
        top: { duration: 3.2, delay: delay + 1.3, repeat: Infinity, ease: "linear" },
        opacity: {
          duration: 3.2,
          delay: delay + 1.3,
          repeat: Infinity,
